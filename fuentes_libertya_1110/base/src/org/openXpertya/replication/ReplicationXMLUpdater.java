...
public class ReplicationXMLUpdater extends PluginXMLUpdater {
	 ...
	 */
	public static void processChangelog(String contentXML, String m_trxName, int AD_Org_ID, int initialChangelogID, int finalChangelogID, boolean delayedInsert) throws Exception
	{
		// Procesar el changelog - Según sea replicación tardía o no, utilizaremos la información del intervalo del changelog 
		if (delayedInsert)
			new ReplicationXMLUpdater(contentXML, m_trxName, AD_Org_ID).processChangeLog();
		else
			new ReplicationXMLUpdater(contentXML, m_trxName, AD_Org_ID, initialChangelogID, finalChangelogID).processChangeLog();

		// Hay que actualizar el dato de ultimo changelog? Si es insercion tardía no hay que hacerlo
		if (delayedInsert)
			return;
	 ...
	 * Se incorpora además al query el campo nochangelog a fin de no bitacorear entradas por replicacion
	 * Por ultimo debe habilitarse el changelog en dicha entrada por modificaciones tradicionales
	 */
	protected void customizeInsertionQuery(StringBuffer sql, ChangeGroup changeGroup)
	{
		...
		sql.insert(lastColumnPos, ",retrieveUID,noChangelog");
		// Insertar en el query los valores de las columnas
		int lastValuePos =  sql.lastIndexOf(")");
		sql.insert(lastValuePos, ",'" + changeGroup.getUid() + "','Y'");
		// Concatenar el UPDATE para noChangelog=N luego del INSERT principal
		sql.append(" UPDATE " + changeGroup.getTableName() + " SET noChangelog = 'N'" + " WHERE retrieveUID = '" + getUniversalReference(changeGroup) + "';");
	}
	...
	{
		// Insertar el campo changelog antes del where
		sql.insert(sql.indexOf("WHERE"), ",noChangelog='Y' ");
		// Concatenar el UPDATE para noChangelog=N luego del UPDATE principal
		sql.append(" UPDATE " + changeGroup.getTableName() + " SET noChangelog = 'N' " + sql.substring(sql.indexOf("WHERE"), sql.length()-1) + ";" );
		
	}
	
	/**
	 *	Se incorpora al final del query el campo nochangelog a fin de no bitacorear entradas por replicacion
	 *  En este caso, primeramente debo setear al registro como noChangelog y recien despues borrarlo
	 */
	protected void customizeDeletionQuery(StringBuffer sql, ChangeGroup changeGroup)
	{
		...
		sql.insert(0, " UPDATE " + changeGroup.getTableName() + " SET noChangelog = 'Y' " + sql.substring(whereClausePos, sql.length()-1) + ";"  );
	}
